<div class="modal-content">
    <form id="formAction" action="{{ $Imunisasi->id ? route('data-imunisasi.update', $Imunisasi->id) : route('data-imunisasi.store') }}" method="POST">
        @csrf
        @if ($Imunisasi->id)
            @method('put')
        @endif
        <div class="modal-header">
            <h5 class="modal-title" id="largeModalLabel">Imunisasi</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"
                aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <input type="hidden" name="id_pasien" value="{{ $Imunisasi->id ? $Imunisasi->pasien->id : $pasien->id }}"> 
            <div class="mb-3">
                <label for="nama_anak" class="form-label">Nama Anak</label>
                <input type="text" placeholder="Nama anak.." value="{{ $Imunisasi->id ? $Imunisasi->pasien->nama_anak : $pasien->nama_anak}}" disabled name="id_pasien" class="form-control" id="id_pasien">
            </div>
            <div class="mb-3">
                <label for="tgl_imunisasi" class="form-label">Tanggal Periksa</label>
                <input type="date" placeholder="Tanggal periksa.." value="{{ $Imunisasi->tgl_imunisasi }}"  name="tgl_imunisasi" class="form-control" id="tgl_imunisasi">
            </div>
            {{-- <div class="mb-3">
            ...
            <div class="col-md-12 mb-3">
                ...
                <select class="form-select " name="id_jenis">
                    ...
                    <option value="{{ $item->id }}" {{ $item->id == $Imunisasi->id_jenis ? 'selected' : '' }} >{{ $item->nama_imunisasi }}</option>
                    {{-- <option value="{{ $item->id }}">{{ $item->nama_imunisasi }}</option> --}}
                    @endforeach
                </select>
            </div>
            {{-- <div class="col-md-12 mb-3">
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary"
                ...
                <button type="submit" class="btn btn-primary">{{ $Imunisasi->id ? 'Simpan perubahan' : 'Simpan' }}</button>
        </div>
    </form>
</div>